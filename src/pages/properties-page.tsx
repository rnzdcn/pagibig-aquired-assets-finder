...
import { useNewListingWatch, WATCH_POLL_MS } from '@/hooks/use-new-listing-watch' // requestNotificationPermission also exported here
import { isSearchReady } from '@/types/filters'
import type { Property } from '@/types/property'

export function PropertiesPage() {
  const { state } = useFiltersUrl() // const { state, update } = useFiltersUrl()
  const { filters, search, auction, view } = state

  const [watchEnabled] = useLocalStorage('pagibig-copa:watch-enabled', false) // const [watchEnabled, setWatchEnabled] = useLocalStorage(...)
